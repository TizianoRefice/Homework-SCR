...
public class NoiseGenerator {
	...
	public void noise(double snr, int length) {
		...
		for(int i = 0; i < this.length; i++) {
			campione = new Random();
			this.parteReale[i] = campione.nextGaussian() * Math.sqrt(potRumore/2);
		}

		this.parteImmaginaria = new double[length];
		for(int j = 0; j < this.length; j++) {
			campione = new Random();
			this.parteImmaginaria[j] = campione.nextGaussian() * Math.sqrt(potRumore/2);
		}
	}
	...
	public double getPotenzaRumore(double[] parteReale, double[] parteImmaginaria) {
		...
		double lunghezza = 1000; 									//indifferente dove la prendo, tanto hanno la stessa lunghezza
